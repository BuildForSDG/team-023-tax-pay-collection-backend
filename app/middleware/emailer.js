...
const sendEmail = async (data, callback) => {
  ...
  const mailOptions = {
    ...
    to: `${data.user.frist_name} <${data.user.email}>`,
...
module.exports = {
  ...
  async sendRegistrationEmailMessage(locale, user) {
    ...
    const htmlMessage = i18n.__('registration.MESSAGE', user.frist_name, process.env.FRONTEND_URL, user.verification);
  ...
  async sendResetPasswordEmailMessage(locale, user) {
    ...
    const htmlMessage = i18n.__('forgotPassword.MESSAGE', user.email, process.env.FRONTEND_URL, user.verification);
    prepareToSendEmail(user, subject, htmlMessage);
  }
};
