...
exports.updateProfile = async (req, res) => {
  try {
    ...
    res.status(200).json(await updateProfileInDB(reqBody, id));
  } catch (error) {
    utils.handleError(res, error);
  }
};
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const reqBody = matchedData(req);
    const isPasswordMatch = await auth.checkPassword(reqBody.oldPassword, user);
    if (!isPasswordMatch) {
      utils.handleError(res, await passwordsDoNotMatch());
    } else {
      // all ok, proceed to change password
      res.status(200).json(await changePasswordInDB(id, reqBody));
    }
  } catch (error) {
    utils.handleError(res, error);
  }
